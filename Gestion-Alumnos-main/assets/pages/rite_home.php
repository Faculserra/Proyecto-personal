...
$resultado_anios = mysqli_query($connection, $query_anios_distintos);
...
<body>
    ...
    <form method="post">
        ...
        <!-- Agrega el menú desplegable de años -->
        <label for="ano">Seleccione el Año Académico:</label>
        <select name="ano" id="ano">
            <option value="todos">Todos los años</option>
            <?php
            // Generar las opciones del menú desplegable con los años disponibles
...
if (isset($_POST['dni'])) {
    $dni = $_POST['dni'];
    $ano = $_POST['ano']; // Capturar el año académico seleccionado por el usuario
    ...
    $resultado_anios = mysqli_query($connection, $query_anios_participacion);
    ...
    foreach ($aniosParticipacion as $anoParticipacion) {
        ...
        foreach ($instancia as $inst) {    
            $consulta_mostrar="SELECT materia.Nombre, curso.Nombre_Curso, curso.Anio, profesores.Nombre_profesor ,profesores.Apellido_profesor, legajo_alumno.Nombre_alumno, legajo_alumno.Apellido_alumno, notas.Fecha_nota, notas.nota_ind1, notas.nota_trayect_1, notas.nota_ind2, notas.nota_trayect_2, notas.nota_ind3, notas.nota_trayect_3, notas.nota_ind4, notas.nota_trayect_4, notas.nota_ind5, notas.nota_trayect_5, notas.nota_ind6, notas.nota_trayect_6, notas.nota_ind7, notas.nota_trayect_7, notas.nota_ind8, notas.nota_trayect_8, notas.nota_ind9, notas.nota_trayect_9, notas.nota_ind10, notas.nota_trayect_10, notas.nota_ind11, notas.nota_trayect_11, notas.nota_ind12, notas.nota_trayect_12 FROM notas INNER JOIN legajo_alumno ON legajo_alumno.Legajo = notas.Alumno INNER JOIN materia ON materia.Id = notas.Materia INNER JOIN curso ON curso.ID_curso = materia.curso INNER JOIN profesores ON profesores.numLegajo = notas.Profesor WHERE notas.Instancia = '$inst' AND legajo_alumno.DNI = '$dni' AND curso.Anio = '$anoParticipacion'";
            echo "<h3>$inst</h3>";
            echo "<table border='1'>
                    <tr>
                        <td>Materia</td>
